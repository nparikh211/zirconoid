...
<header class="nav${home ? '' : ' nav--sticky'}" data-nav>
  ...
  <nav class="nav__links" aria-label="Primary">
    ...
    <a class="nav__link" href="${rel}samples/"${current === 'samples' ? ' aria-current="page"' : ''}>Sample Datasets</a>
    <button type="button" class="btn" data-sample-open>${STAR}Talk to a Data Expert</button>
  </nav>
</header>`;
}
...
function footerLinks(rel, current) {
  ...
  return `
      ...
      <a href="${rel}samples/"${cur('samples')}>Sample Datasets</a>
...
export function layout({
  ...
  published = '', modified = SITE.updated, extraHead = '', extraScripts = '',
}) {
...
<link rel="stylesheet" href="${rel}${v('assets/css/sample-modal.css')}">
...
<div class="sample-modal" data-sample-modal id="sample-dialog" hidden>
  ...
  <div class="sample-modal__panel" role="dialog" aria-modal="true" aria-labelledby="sample-dialog-title" data-sample-panel tabindex="-1">
    ...
    <div data-sample-form-wrap>
      <h2 class="sample-modal__title" id="sample-dialog-title">Talk to a Data Expert</h2>
      <p class="sample-modal__lede">Tell us the domain and volume you need. We will return a scoped sample and a capture plan.</p>
      <form class="sample-form" data-sample-form action="https://formsubmit.co/ajax/${SITE.email}" method="POST" novalidate>
        ...
        <label class="sample-form__field">
          ...
          <textarea name="message" required rows="4" placeholder="Domain, task, and volume"></textarea>
        </label>
        <p class="sample-form__error" data-sample-error hidden role="alert"></p>
        <button type="submit" class="btn" data-sample-submit>${STAR}Talk to a Data Expert</button>
      </form>
    </div>
...
<script src="${rel}${v('assets/js/site.js')}" defer></script>
<script src="${rel}${v('assets/js/sample-modal.js')}" defer></script>${home ? `\n<script type="module" src="${rel}${v('assets/js/galaxy.js')}"></script>` : ''}${paper ? `\n<script type="module" src="${rel}${v('assets/js/paper.js')}"></script>` : ''}${extraScripts}
</body>
</html>
`;
}
