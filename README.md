...
- **Galaxy.** `<zirconoid-galaxy>` is a custom element that renders a GPGPU particle galaxy with three.js. The hero copy sits in a masked layer above the nav (`z-index: 60`) so the nav blur never touches it, and it moves at 0.62x scroll speed for parallax. The footer copy has mouse tilt off, spins slower and is dimmed to 45%. A perspective camera crops horizontally as the frame narrows, so `resize()` scales the spiral down by the frame's aspect and eases its offset back towards the centre by the same amount; without that the phone showed a sliver of its middle. Under 640px the layer is also wider than the screen, shorter, softly masked and held at 78% opacity, so it sits above the headline instead of behind it.
...
- **Definition on a touch screen.** Both Zirconoid marks show their definition on hover, which a phone has none of, and the footer mark is also a link home, so a tap there used to jump to the top of the page. Under `(hover: none)` a tap opens the definition instead and never follows the link; a tap anywhere else closes it, and only one is open at a time.
