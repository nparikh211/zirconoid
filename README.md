...
| `/buy/` | `src/pages/buy.js` — URL-only (not linked from nav, footer, or homepage); Stripe scaffold + checkout copy |
| `/data-license/` | `src/pages/data-license.js` + `docs/DATA_LICENSE.md` — URL-only; linked from `/buy/` only |
| `/terms-of-sale/` | `src/pages/terms-of-sale.js` + `docs/TERMS_OF_SALE.md` — URL-only; linked from `/buy/` only |
...
- **Definition bubble.** Both Zirconoid marks, the one over the belief text and the one in the footer, are centred on their column and show the same definition above themselves on hover. A phone has no hover, so under `(hover: none)` a tap opens it instead, a tap anywhere else closes it, and only one is open at a time. On a narrow screen the bubble is held to the page gutters so it never runs off an edge.
...
- **Ink.** Grain breaks up a letter the way a flat background never does, so the text is black rather than the dark theme's off-white inverted, the supporting greys are far darker than their dark-theme counterparts, and the body copy carries normal weight instead of the light weight it uses on screen.
...
- Commercial buy page (`/buy/`, URL-only): `src/pages/buy.js`, Stripe placeholders in `public/assets/js/buy-config.js`, client stub in `public/assets/js/buy.js`. Full license/terms live in `docs/DATA_LICENSE.md` and `docs/TERMS_OF_SALE.md`, rendered at `/data-license/` and `/terms-of-sale/` (also URL-only; linked from `/buy/` only).
