...
- **Trusted by.** The lab names orbit the claim: the ring turns once every 96 seconds and each name turns back by the same amount, so it stays upright. They are wordmarks set in the site's own mono, not brand logos, which keeps them monochrome and consistent and avoids reproducing marks we cannot source. Give a brand an `svg` in `FRONTIER_LABS` (`src/pages/home.js`) and it renders in place of the name. Under 720px the circle is dropped for a plain row. The labs are deliberately absent from the structured data, since they are not Zirconoid customers; a test enforces that.
